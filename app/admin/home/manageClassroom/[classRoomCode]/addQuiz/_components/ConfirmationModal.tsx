...
export default function ConfirmationModal({
	...
	const handleInputChange = async (
		...
		if (type === "hours") {
			await setHours(Math.min(Math.max(value, 0), 23));
		} else if (type === "minutes" || type === "seconds") {
			if (value >= 0 && value < 60) {
				type === "minutes"
					? await setMinutes(value)
					: await setSeconds(value);
			}
		}
	};
	...
	const handleQuizCreation = async (
		...
		try {
			...
			const timeDurationString = dayjs()
				.hour(hours)
				.minute(minutes)
				.second(seconds)
				.format("HH:mm:ss");
      ...
      newQuizDetails.timeDuration = timeDurationString;
