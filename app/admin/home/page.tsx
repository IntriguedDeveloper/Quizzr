...
export default function ClassroomViewer() {
		...
		async function fetchClasses(teacherName: string) {
			try {
				...
				for (const classDoc of classesSnapshot.docs) {
					...
					const relevantTeacherDocSnapShot = await getDoc(
						teacherDocRef
					);
					if (relevantTeacherDocSnapShot.exists()) {
						const obj: ClassObject = {
							...
							optedSubject:
								relevantTeacherDocSnapShot.data()
									.selectedSubject,
						};
	...
	return (
		<div className="px-2 py-4 space-y-6 flex-col flex justify-center items-center lg:w-5/6 w-full flex-shrink-0">
					...
					{relevantClasses.length > 0 ? (
						relevantClasses.map((classObject: ClassObject) => (
							<ClassCard
								...
								ClassName={
									classObject.classDataObject.className
								}
								ClassCode={
									classObject.classDataObject.classCode
								}
			...
			{isActive && (
				<Dropdown refreshClassesSetter={refreshClassesSetter} />
			)}
		</div>
	);
}
