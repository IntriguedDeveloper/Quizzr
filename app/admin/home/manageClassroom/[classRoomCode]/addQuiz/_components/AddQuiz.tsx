  ...
  import { mcqExtraction } from "../actions";
  ...
  export default function AddQuiz() {
    ...
    const[picturesArray, setPicturesArray] = useState<File[]>([]);
    ...
    const handleAIFileUpload = async (
      event: React.ChangeEvent<HTMLInputElement>
    ) => {
      const file = event.target.files?.[0];
      if (!file) return;
      if (file.name.endsWith(".pdf")) {
        const response = await mcqExtraction(file);
        console.log(response);
        if (response.content) {
          setQuestionsArray(response.content);
          toast.success("Added questions from file!")
        }
        if (response.error) {
        ...
        <div className="w-full max-w-5xl mx-auto bg-white rounded-lg shadow-xl">
              ...
              <div className="space-y-6">
                ...
                >
                  <QuestionCard
                    ...
                    setPicturesArray = {(picturesArray: File[]) => setPicturesArray(picturesArray)}
                  />
                </div>

                <div className="flex justify-between items-center mt-6">
                  <button
                    onClick={() => previousQuestionTransition(renderIndex)}
                  ...
                  <div className="text-sm font-medium text-gray-500">
                    Question {renderIndex + 1} of {questionsArray.length}
                  </div>

                  <button
                    onClick={() => nextQuestionTransition(renderIndex)}
          ...
          {showConfirmationModal && (
            <ConfirmationModal
              ...
              questionPictures={picturesArray}
            />
          )}
        </div>
      </>
